...
If we are going to allow input from a user to choose a range for generating our number, we will need to figure out how many bits are required to represent a given base 10 integer so we know how many random bits to generate. The equation we need to do this is $b_{spec} = \left \lfloor log_2(n) \right \rfloor + 1$[¹²](#ref-12), which we can represent in python as the function:
